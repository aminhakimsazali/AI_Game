...
//                node = new Ucb1SelectionPolicy().selectChild(node);
                ...
                while (!game.isJunction(game.getPacmanCurrentNodeIndex())) {
                    game.advanceGame(MonteCarloPacMan.nonJunctionSim(game), ghosts.getMove(game, 40));
                }
...
//                node = new Ucb1SelectionPolicy().selectChild(node);
    ...
    public double getUcbValue(MonteCarloGameNode node) {
        final double DEFAULT_BALANCE = 10000;
        return node.getAverageScore() + DEFAULT_BALANCE *
